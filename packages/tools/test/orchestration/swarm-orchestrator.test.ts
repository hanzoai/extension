...
import { AgentSwarmManager, AgentInstance, AgentSwarmConfig } from '../../src/config/agent-swarm-config';
import { CLIToolManager } from '../../src/cli-tool-manager';
import * as child_process from 'child_process';
import * as path from 'path';

vi.mock('../../src/config/agent-swarm-config');
vi.mock('../../src/cli-tool-manager');
...
describe('SwarmOrchestrator', () => {
  ...
  beforeEach(() => {
    ...
    // Mock process.chdir as it's not available in worker threads
  ...
  describe('task execution', () => {
    ...
    it('should handle delegated tasks', async () => {
      ...
      // Should have results from leader and delegated tasks
      expect(results.length).toBeGreaterThanOrEqual(1);
      
      // Find the leader result (might not be first if delegated tasks complete first)
      const leaderResult = results.find(r => r.agentName === 'leader');
      expect(leaderResult).toBeDefined();
      expect(leaderResult?.result).toContain('I will delegate this');
    });
    ...
    it('should change to agent directory during execution', async () => {
      ...
      // Check that process.chdir was called with the agent's directory
      expect(process.chdir).toHaveBeenCalledWith(path.resolve(originalCwd, './src'));
      expect(process.chdir).toHaveBeenCalledWith(originalCwd); // Called to restore
    });
    ...
    it('should emit agent lifecycle events', async () => {
      ...
      // The test might have emitted either complete or error event
      if (completeListener.mock.calls.length > 0) {
  ...
  describe('status reporting', () => {
    ...
    it('should track active agents during execution', async () => {
      ...
      // Status should be 'ready' as active tracking happens internally
      expect(statusDuringExecution.agents.leader.status).toBe('ready');
    });
  });
  ...
  describe('shutdown', () => {
    ...
    it('should clear all internal state', async () => {
      ...
      expect(statusAfter.results).toHaveLength(0); // Results cleared
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should handle invalid agent names in delegation', async () => {
      ...
      // Should have one result from the main agent
      expect(results).toHaveLength(1);
