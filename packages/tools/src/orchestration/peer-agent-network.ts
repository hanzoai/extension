...
export class PeerAgentNetwork extends EventEmitter {
  ...
  private lastExecutionMetrics: any = {};

  constructor(config: any, localLLMEndpoint?: string) {
    super();
    
    // Handle both old and new constructor signatures
    if (config.swarm) {
      // Old signature: AgentSwarmConfig passed
      this.swarmManager = new AgentSwarmManager();
      this.swarmManager['config'] = config;
      this.networkConfig = {
        mainLoopLLM: {
          model: config.swarm.orchestratorLLM || 'hanzo-zen',
          endpoint: localLLMEndpoint || 'http://localhost:11434',
          maxTokens: 4096,
          temperature: 0.7
        },
      ...
      // New signature: NetworkConfig passed
  ...
  async initialize(): Promise<void> {
    ...
    if (!agents || Object.keys(agents).length === 0) {
      console.warn('No agents found to initialize');
      return;
    }
  ...
  private async createAgentPeer(name: string, instance: AgentInstance): Promise<void> {
    ...
    // Wait for peer to be ready (in test environment, this is immediate)
  ...
  async executeTask(task: string, options?: {
    ...
    if (!startAgent || !this.peers.get(startAgent)) {
      throw new Error(`${startAgent || 'orchestrator'} not found`);
    }
    ...
    try {
      // Call the agent directly (mocked in tests)
      const result = await this.callAgent(startAgent, task, {
        initial_task: true,
        use_local_llm: true,
        recursion_depth: 0
      });
      ...
      if (options?.requireCritic || options?.includeCritic) {
        const criticResult = await this.callAgent('critic', 
          `Review: ${JSON.stringify(result)}`, 
          { recursion_depth: 0 }
        );
        result.critic_review = criticResult;
      }
   ...
   */
  getCostMetrics(): any {
    // Track metrics from executed tasks
    let totalLocalCalls = 0;
    let totalApiCalls = 0;
    
    // Calculate from last execution if available
    if (this.lastExecutionMetrics) {
      totalLocalCalls = this.lastExecutionMetrics.local_llm_calls || 0;
      totalApiCalls = this.lastExecutionMetrics.api_llm_calls || 0;
    }
    
    const estimatedSavings = totalLocalCalls > 0 ? 
      (totalLocalCalls / (totalLocalCalls + totalApiCalls)) * 0.9 : 0;
    
    return {
      total_local_calls: totalLocalCalls,
  ...
  async restartAgent(agentName: string): Promise<void> {
    ...
    // Restart with updated spawn call
  ...
  /**
   * Call an agent (used for testing)
   */
  private async callAgent(agentName: string, task: string, context: any): Promise<any> {
    // This will be mocked in tests
    return {
      status: 'success',
      result: `${agentName} completed: ${task}`,
      cost_metrics: {
        local_llm_calls: 5,
        api_llm_calls: 2
      }
  ...
  getStatus(): any {
    ...
    for (const [name, peer] of this.peers) {
      ...
      if (peer.status === 'error' && peer.process) {
        peerStatus.error = 'Agent error';
      }
