...
export class ApiClient {
        ...
        if (options.shouldGzip) {
            ...
            const makeRequest = async (): Promise<AxiosResponse> => {
                try {
                    ...
                    const response = await axios.post(`${this.config.apiUrl}${endpoint}`, body, {
     ...
     */
    private gzipContent(content: string): string {
     ...
     */
    private async makeChunkedRequest(
        endpoint: string,
        originalData: any,
        token: string | null,
        options: ApiClientOptions
        ...
        const makeChunkRequest = async (chunkIndex: number, filesChunk: any[]): Promise<AxiosResponse> => {
            ...
            try {
                const response = await axios.post(`${this.config.apiUrl}${endpoint}`, chunkJsonBody, {
                    headers,
                    httpsAgent: new (require('https').Agent)({ rejectUnauthorized: false }),
                    timeout: GLOBAL_REQUEST_TIMEOUT
                });
     ...
     */
    private combineChunkResponses(responses: AxiosResponse[]): AxiosResponse {
        ...
        const combinedResponse: AxiosResponse = {
            ...baseResponse,
     ...
     * @returns API response
     */
    private async makeGzippedChunkedRequest(
        endpoint: string,
        originalData: any,
        token: string | null,
        ...
        const makeGzippedChunkRequest = async (chunkIndex: number, filesChunk: any[]): Promise<AxiosResponse> => {
            ...
            try {
                const response = await axios.post(`${this.config.apiUrl}${endpoint}`, compressedChunkBody, {
                    headers,
                    httpsAgent: new (require('https').Agent)({ rejectUnauthorized: false }),
                    timeout: GLOBAL_REQUEST_TIMEOUT
                });
     ...
     */
    private chunkArray(array: any[], maxChunkSize: number): any[][] {
        const chunks: any[][] = [];
